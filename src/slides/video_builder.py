...
def build_tiktok_video(
    ...
    """Build a narrated TikTok video from a standalone script (130-160 words).

    Unlike build_video which renders per-slide clips, this creates a single
    continuous video with the full script as one TTS narration over a
    simple branded background.

    Args:
        script_text: The full TikTok script text (hook + body + CTA).
        topic: Topic title for the background card.
        colors: Color scheme dict (background, title, body, accent, highlight).
        aspect_ratio: '9:16' (default for TikTok) or '16:9'.
        output_dir: Where to save the MP4.
        handle: Social media handle watermark.
        voice_id: ElevenLabs voice ID.
    ...
    # Step 1: Render a simple branded background with the topic title
    from PIL import Image, ImageDraw, ImageFont

    bg = Image.new("RGB", (img_w, img_h), colors["background"])
    draw = ImageDraw.Draw(bg)

    # Draw handle watermark at the bottom
    try:
        font_sm = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 28)
    except (OSError, IOError):
        font_sm = ImageFont.load_default()
    handle_bbox = draw.textbbox((0, 0), handle, font=font_sm)
    handle_w = handle_bbox[2] - handle_bbox[0]
    draw.text(
        ((img_w - handle_w) // 2, img_h - 80),
        handle,
        fill=colors.get("body", "#C0C0D0"),
        font=font_sm,
    )

    # Draw topic title centered
    if topic:
        try:
        ...
        for word in words:
            ...
            test_bbox = draw.textbbox((0, 0), test, font=font_lg)
        ...
        for i, line in enumerate(lines):
            line_bbox = draw.textbbox((0, 0), line, font=font_lg)
            line_w = line_bbox[2] - line_bbox[0]
            draw.text(
                ((img_w - line_w) // 2, y_start + i * line_height),
                line,
                fill=colors.get("title", "#F0F0F0"),
                font=font_lg,
            )

    bg_path = os.path.join(output_dir, "tiktok_bg.png")
    bg.save(bg_path)
    bg.close()

    # Step 2: Synthesize full script as one TTS clip
    with tempfile.TemporaryDirectory() as tmp_dir:
        audio_path = os.path.join(tmp_dir, "tiktok_narration.mp3")
        audio_bytes = synthesize_speech(text=script_text, voice_id=voice_id)
        with open(audio_path, "wb") as f:
            f.write(audio_bytes)
        del audio_bytes
        ...
        # Add 0.5s pre-roll + 1.0s post-roll silence
        pre_roll = 0.5
        post_roll = 1.0
        padded_audio = CompositeAudioClip(
            [audio_clip.with_start(pre_roll)]
        ).with_duration(audio_clip.duration + pre_roll + post_roll)

        total_duration = padded_audio.duration

        # Step 3: Create video — background image + narration
        img_clip = (
            ImageClip(bg_path)
            .with_duration(total_duration)
            .with_audio(padded_audio)
        )

        # Step 4: Write MP4
        output_path = os.path.join(output_dir, "tiktok_script.mp4")
        ...
        img_clip.close()
        audio_clip.close()
        del img_clip, padded_audio, audio_clip
        gc.collect()
