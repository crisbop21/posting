...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with studio_tabs[3]:
        ...
        if not video_enabled:
            ...
            fallback_label = ""
            if openai_img_key:
                fallback_label = " If no web images are found, DALL-E generates a replacement."
            st.caption(
                f"Slides use web image backgrounds ({provider_hint}).{fallback_label}"
            )
            ...
            if st.session_state.video_scripts:
                ...
                if st.button("Build Narrated Video", type="primary", use_container_width=True, key="build_video"):
                    ...
                    # Set DALL-E key so image_search can fall back to it
                    if openai_img_key:
                        os.environ["OPENAI_API_KEY"] = openai_img_key
                    ...
                    with st.spinner("Searching for images and building video..."):
                        try:
